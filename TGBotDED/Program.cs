...
async Task HandleMessage(ITelegramBotClient botClient, Update update, Message message)
{
    string Hello = $"Привет! Я бот \"{botClient.GetMeAsync().Result.FirstName}\"\n\n" +
        ...
        $"\nВот тебе ссылочка, с подробным описанием меня!😉";
    ...
    if (message.Chat.Type != null)
    {
        ...
        if (message.Text.StartsWith("/info"))
        {
            ...
            try
            {
                ...
                {
                    MySqlBase.Open();
                    string cmdsql = $"SELECT market FROM BDGroup WHERE id = '{message.Chat.Id}';";
                    MySqlCommand commandMarket = new MySqlCommand(cmdsql, MySqlBase);
                    string Market = commandMarket.ExecuteScalar().ToString();
                    MySqlBase.Close();
                    TextMes = $"{TextMes}\n\n" +
        ...
        if (message.Text.StartsWith("/bd_show"))
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    Text = $"{Text}{id}|{title}|{type}|{market}\n";
                }
        ...
        if (message.Text.StartsWith("/help"))
        {
            string Text = $"Список основных команд:\n" +
                ...
                $"\n" +
                $"/tiket_svo_try - авиабилеты в Анталию;\n" +
                $"/bidon - где Байден!?;\n" +
